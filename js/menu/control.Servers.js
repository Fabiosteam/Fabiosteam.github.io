...
function ControllerServers( $scope, $element, $rootScope, $location )
{
	...
	if ( !Scope.TopServers ) Scope.TopServers = null;
	...
	$scope.SelectServer = function( server, event )
	{
		...
		if ( !IN_ENGINE )
			SetPlayerList( server.address, { "1": { "time": 10000000000.74, "score": 5, "name": "Sethxi" }, "2": { "time": 2029.34, "score": 0, "name": "RedDragon124" }, "3": { "time": 1405.02, "score": 0, "name": "Joke (0_0)" }, "4": { "time": 462.15, "score": 0, "name": "TheAimBot" }, "5": { "time": 301.32, "score": 0, "name": "DesanPL"},"6": { "time": 301.32, "score": 0, "name": "DesanPL"},"7": { "time": 301.32, "score": 0, "name": "DesanPL"},"8": { "time": 301.32, "score": 0, "name": "DesanPL"},"9": { "time": 301.32, "score": 0, "name": "DesanPL"},"10": { "time": 301.32, "score": 0, "name": "DesanPL"} } );
	...
	$scope.TopServera = function()
	{
		Scope.TopServers = true;
	...
	$scope.ServerRankInt = function( sv )
	{
		if ( sv.recommended < 50 )	return 5;
		if ( sv.recommended < 100 )	return 4;
		if ( sv.recommended < 200 )	return 3;
		if ( sv.recommended < 300 )	return 2;
		return 1;
	}
...
function UpdateInfiniteScroll( elem )
{
	...
	Scope.CurrentGamemode.server_offset = Math.max( Math.floor( elem.scrollTop / 64 ) - ( Scope.ServersPerPage / 4 ), 0 );
...
function AddServer( type, id, ping, name, desc, map, players, maxplayers, botplayers, pass, lastplayed, address, gamemode, workshopid, isAnon, version, isFav, loc, gmcat )
{
	...
	if ( !IN_ENGINE ) GMOD_VERSION_INT = 20200101; // if ( !IN_ENGINE ) GMOD_VERSION_INT = 200101;
