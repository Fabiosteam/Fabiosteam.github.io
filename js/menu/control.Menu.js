...
function MenuController( $scope, $rootScope )
{
	...
	$scope.ToggleServers = function(name)
	{
		$( '.popup:not(.servermenu)' ).hide();
		$( '.servermenu' ).toggle();
		if (name == undefined) return;

		$scope.SelectedServer = $rootScope.InfoProject['projects'][name];
		document.getElementById('serverLogo').src = $rootScope.InfoProject['projects'][name]['logo']
	}
	
	$scope.ConnectServer = function(ip)
	{
	...
	$scope.SetClipboardText = function()
	{
		lua.Run( "SetClipboardText( %s )", $scope.CurrentGamemode.Selected.address );
	}
	...
	$scope.SumOnline = function( project )
	{
		if(!project){
			return -1;
		}
		return project.servers.reduce( function( prev, curr ){
			if(curr && curr.players) {
				return curr.players + prev
			} else {
				return prev
			}
		}, 0);
...
function UpdateVersionNew( version, netVersion, branch )
{
