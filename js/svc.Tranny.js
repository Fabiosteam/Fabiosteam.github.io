...
.directive( 'ngSeconds', function ( $parse )
{
	return function ( scope, element, attrs )
	{
		...
		{
			if ( value < 60 )
				return $(element).html( Math.floor( value ) + " сек." );

			if ( value < 60 * 60 )
				return $( element ).html( Math.floor( value / 60 ) + " мин." );

			if ( value < 60 * 60 * 24 )
				return $( element ).html( Math.floor( value / 60 / 60 ) + " часов" );

			$( element ).html( "п**дец долго чел сидит" );
